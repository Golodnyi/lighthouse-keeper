        ...
        if let UpdateKind::Message(message) = update.kind {
            ...
            if ChatId::new(0) < chat_id {
                api.spawn(message.text_reply(
                    format!("Морти! Я работую только в групповых чатах!\nВ основном я занимаюсь подсчетом количества сообщений пользователя и фиксацией даты последней активности, но, если мне дать права администратора (с функцией кика), я смогу выгонять из чата всех, кто молчал больше месяца. Я называю это судная ночь!")
                ));
